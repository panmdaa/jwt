...
describe("verify", () => {
	it("rejects malformed tokens with missing or non-JSON segments", () => {
		expect(() => verify("abc", "secret", { algorithms: ["HS256"] })).toThrow(
			MalformedToken,
		);
		expect(() => verify("a.b.c", "secret", { algorithms: ["HS256"] })).toThrow(
			MalformedToken,
		);
	});
	...
	it("rejects disallowed algorithms before verifying the key", () => {
		...
		expect(() => verify(token, "secret", { algorithms: ["HS384"] })).toThrow(
			AlgorithmNotAllowed,
		);
		const noneHeader = Buffer.from(
			JSON.stringify({ alg: "none", typ: "JWT" }),
		).toString("base64url");
		const nonePayload = Buffer.from(JSON.stringify({ sub: "user" })).toString(
			"base64url",
		);
		const noneToken = `${noneHeader}.${nonePayload}.Zm9v`;
		expect(() =>
			verify(noneToken, "secret", { algorithms: ["HS256"] }),
		).toThrow(AlgorithmNotAllowed);
	});
	...
	it("validates exp and nbf against clock tolerance", () => {
		...
		expect(() =>
			verify(expiredToken, "secret", { algorithms: ["HS256"] }),
		).toThrow(TokenExpired);

		const futureToken = sign({ nbf: now + 300 }, "secret", { alg: "HS256" });
		expect(() =>
			verify(futureToken, "secret", { algorithms: ["HS256"] }),
		).toThrow(TokenNotYetValid);

		const tolerantToken = sign({ exp: now + 10, nbf: now + 10 }, "secret", {
			alg: "HS256",
			expiresIn: "1h",
		});
		expect(() =>
			verify(tolerantToken, "secret", {
				algorithms: ["HS256"],
				clockTolerance: 20,
			}),
		).not.toThrow();
	...
	it("validates audience claims when audience is configured", () => {
		...
		expect(() =>
			verify(token, "secret", { algorithms: ["HS256"], audience: "web" }),
		).toThrow(AudienceMismatch);
		expect(() =>
			verify(token, "secret", {
				algorithms: ["HS256"],
				audience: ["web", "app"],
	...
	it("validates issuer claims when issuer is configured", () => {
		...
		expect(() =>
			verify(token, "secret", { algorithms: ["HS256"], issuer: "issuer-b" }),
		).toThrow(IssuerMismatch);
	});

	it("validates subject claims when subject is configured", () => {
		const token = sign({ sub: "user-a" }, "secret", { alg: "HS256" });
		expect(() =>
			verify(token, "secret", { algorithms: ["HS256"], subject: "user-b" }),
		).toThrow(SubjectMismatch);
	});
	...
	it("returns the payload and optional complete object when requested", () => {
		...
		const complete = verify(token, "secret", {
			algorithms: ["HS256"],
			complete: true,
		});
	...
	it("rejects tokens exceeding maxTokenLength before any parsing", () => {
		...
		expect(() =>
			verify(longToken, "secret", {
				algorithms: ["HS256"],
				maxTokenLength: 2048,
			}),
		).toThrow(MalformedToken);
	});
	...
	it("supports RSA and ECDSA verification paths with explicit algorithm allowlists", () => {
		...
		expect(
			verify(rsaToken, publicKey, { algorithms: ["RS256"] }),
		).toMatchObject({ sub: "rsa-user" });

		const { privateKey: ecdsaPrivate, publicKey: ecdsaPublic } =
			generateKeyPairSync("ec", { namedCurve: "prime256v1" });
		const ecdsaToken = sign({ sub: "ec-user" }, ecdsaPrivate, { alg: "ES256" });
		expect(
			verify(ecdsaToken, ecdsaPublic, { algorithms: ["ES256"] }),
		).toMatchObject({ sub: "ec-user" });
	});
});
