...
function parseJsonObject(
	...
	try {
		...
		throw new MalformedToken(
			`Token ${label} is not valid JSON`,
			`The ${label} segment does not decode to a valid JSON object`,
		);
	}
}
...
export function verify(
	...
	if (typeof token !== "string") {
		throw new MalformedToken(
			...
			"JWT verification requires a string token",
		);
	}
	...
	if (parts.length !== 3) {
		throw new MalformedToken(
			"Token is malformed",
			"JWT tokens must contain exactly three dot-separated segments",
		);
	}
	...
	if (!headerSegment || !payloadSegment || !signatureSegment) {
		throw new MalformedToken(
			"Token is malformed",
			"JWT segments cannot be empty",
		);
	}

	const header = parseJsonObject(
		Buffer.from(decodeBase64Url(headerSegment)).toString("utf8"),
		"header",
	);
	const payload = parseJsonObject(
		Buffer.from(decodeBase64Url(payloadSegment)).toString("utf8"),
		"payload",
	);
	...
	if (typeof headerAlg !== "string") {
		throw new AlgorithmNotAllowed(
			"JWT header 'alg' is missing or invalid",
			"JWT headers must include a string 'alg' claim",
		);
	}
	...
	if (headerAlg === "none") {
		throw new AlgorithmNotAllowed(
			...
			"JWT 'none' must be rejected explicitly",
		);
	}
	...
	if (!isValid) {
		throw new InvalidSignature(
			...
			"The JWT signature does not match the supplied key",
		);
	}
	...
	if (
		...
		isExpired(payload.exp, currentTime, clockTolerance)
	) {
		throw new TokenExpired(
			"Token has expired",
			`The exp claim (${payload.exp}) is no longer valid`,
		);
	}
	if (
		typeof payload.nbf === "number" &&
		isNotYetValid(payload.nbf, currentTime, clockTolerance)
	) {
		throw new TokenNotYetValid(
			"Token is not yet valid",
			`The nbf claim (${payload.nbf}) is in the future`,
		);
	}

	if (options.audience !== undefined) {
		const expectedAudience = normalizeAudienceToSet(options.audience);
		const providedAudience = normalizeAudienceToSet(
		...
		const hasMatch = Array.from(providedAudience).some((value) =>
			expectedAudience.has(value),
		);
		if (!hasMatch) {
			throw new AudienceMismatch(
				"Token audience claim does not match expected value",
				"The JWT audience does not match the configured audience",
			);
		}
	}
	...
	if (options.issuer !== undefined && payload.iss !== options.issuer) {
		throw new IssuerMismatch(
			...
			`Expected issuer '${options.issuer}' but received '${String(payload.iss ?? "undefined")}'`,
		);
	}

	if (options.subject !== undefined && payload.sub !== options.subject) {
		throw new SubjectMismatch(
			"Token subject claim does not match expected value",
			`Expected subject '${options.subject}' but received '${String(payload.sub ?? "undefined")}'`,
		);
	}
